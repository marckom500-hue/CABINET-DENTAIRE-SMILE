...
//   const { patients } = usePatients()
//   const { factures, encaisse } = useFactures()
...
//   // 🔥 Patients récents (optimisé)
//   const recentPatients = useMemo(() => {
//     const map = new Map()

//     for (let rdv of rendezVous) {
//       if (!rdv.patients) continue
//       const key = rdv.patient_id
//       if (!map.has(key)) {
//         map.set(key, {
//           id: key,
//           nom: rdv.patients.nom,
...
//       },
//     ]
//   }, [patients, todayAppointments, encaisse])
...
import { useAuthContext } from '../hooks/AuthContext'
import { useNotifications } from '../hooks/NotificationsContext'
import { normalizeRdvStatus, RDV_STATUS, RDV_STATUS_META } from '../lib/statuses'
...
function Dashboard() {
  ...
  const rdvStats = useMemo(() => {
    ...
    rendezVous.forEach(rdv => {
      ...
      if (statut === RDV_STATUS.TERMINE && rdv.patient_present === true) {
        honores++
      } else if (statut === RDV_STATUS.ANNULE) {
        manques++
      } else if (statut === RDV_STATUS.TERMINE && rdv.patient_present === false) {
        manques++
      }
    })

    return { honores, manques, total: honores + manques }
  }, [rendezVous])

  // 🔥 Taux de présence
  const tauxPresence = useMemo(() => {
    if (rdvStats.total === 0) return 0
    return Math.round((rdvStats.honores / rdvStats.total) * 100)
  }, [rdvStats])
  ...
  const allNotifications = useMemo(() => {
    return [
      ...
      ...missedReminders.map(r => ({
        id: `missed-${r.id}`,
        type: 'error',
        message: `Rappel SMS échoué`,
        time: new Date(r.created_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
      })),
    ]
  }, [notifications, missedReminders])

  // 🔥 RDV du jour (pour AppointmentList)
  const todayAppointments = useMemo(() => {
    return rendezVous
      .filter(r => {
        const rdvDate = r.date?.split('T')[0]
        return rdvDate === today
      })
      .map(r => ({
        id: r.id,
  ...
  // 🔥 Répartition des actes (pour DonutChart)
  const actesData = useMemo(() => {
    const actesCount = {}
    rendezVous.forEach(rdv => {
      if (rdv.type_acte && normalizeRdvStatus(rdv.statut) !== RDV_STATUS.ANNULE) {
        actesCount[rdv.type_acte] = (actesCount[rdv.type_acte] || 0) + 1
      }
    })
    return Object.entries(actesCount)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6)
      .map(([label, value], index) => ({
        label,
  ...
  // 🔥 Patients récents (reçus)
  const recentPatients = useMemo(() => {
    const visitedPatients = new Map()
    rendezVous
      .filter(r => r.patients && normalizeRdvStatus(r.statut) === RDV_STATUS.TERMINE && r.patient_present === true)
      .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0))
      .forEach(rdv => {
        const key = rdv.patient_id
        if (!visitedPatients.has(key)) {
          visitedPatients.set(key, {
            id: key,
            nom: rdv.patients.nom,
            prenom: rdv.patients.prenom,
            acte: rdv.type_acte,
    ...
    return Array.from(visitedPatients.values()).slice(0, 8)
  }, [rendezVous])
  ...
  const kpiData = [
    {
      ...
      label: 'Patients total',
      value: patients.length,
      trend: `${recentPatients.length} reçus récemment`,
      trendUp: true,
      color: 'teal',
    },
    {
      id: 2,
      label: 'RDV aujourd\'hui',
      value: todayAppointments.filter(r => normalizeRdvStatus(r.status) === RDV_STATUS.PROGRAMME).length,
      trend: `${todayAppointments.filter(r => normalizeRdvStatus(r.status) === RDV_STATUS.TERMINE).length} terminés`,
      trendUp: null,
      color: 'blue',
    },
    {
      id: 3,
      label: 'Chiffre d\'affaires',
      value: `${encaisse.toLocaleString('fr-FR')} FCFA`,
      trend: `${factures.reduce((sum, f) => sum + (Number(f.montant) || 0), 0).toLocaleString('fr-FR')} FCFA total`,
      trendUp: true,
      color: 'green',
    },
    {
      id: 4,
      label: 'RDV honorés / manqués',
      value: `${rdvStats.honores} / ${rdvStats.manques}`,
      trend: `${tauxPresence}% de présence`,
      trendUp: rdvStats.honores >= rdvStats.manques,
      color: 'indigo',
    },
  ]

  return (
    <div className="space-y-4 pb-20 lg:pb-0 lg:h-[calc(100vh-4rem)] lg:overflow-y-auto">
      <Topbar />
      ...
      {/* Grille principale - comme sur l'image */}
      <div className="grid grid-cols-1 lg:grid-cols-3 lg:grid-rows-2 gap-3 lg:h-[calc(100%-9.25rem)] lg:min-h-0">
