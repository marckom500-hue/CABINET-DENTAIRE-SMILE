...
-- STEP 3: Supprimer l'ancienne contrainte si elle existe (pour éviter les erreurs)
DO $$
BEGIN
  ALTER TABLE rendez_vous DROP CONSTRAINT IF EXISTS chk_termine_has_presence;
EXCEPTION WHEN others THEN
  NULL;
END $$;

-- STEP 4: Ajouter la nouvelle contrainte: Si terminé, patient_present ne peut pas être NULL
ALTER TABLE rendez_vous ADD CONSTRAINT chk_termine_has_presence
  CHECK (statut != 'terminé' OR patient_present IS NOT NULL);

-- STEP 5: Créer un trigger pour mettre à jour updated_at automatiquement
...
-- STEP 6: Créer des index pour les requêtes fréquentes
...
-- STEP 7: Vérifier l'intégrité des données
SELECT 
  COUNT(*) as total_rdv,
...
-- STEP 8: Afficher les RDV avec potential issues
SELECT 
  id,
  date,
  heure,
