...
export default function PatientList({ patients = [] }) {
  return (
    <div className="rounded-xl border p-4 h-full min-h-0 flex flex-col" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-1)' }}>
      <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-1)' }}>Patients reçus</h3>
      <div className="space-y-2 overflow-y-auto pr-1">
        {patients.length === 0 && (
          <div className="text-sm text-center py-8" style={{ color: 'var(--text-3)' }}>Aucun patient reçu récemment</div>
        )}
        {patients.map(p => (
          <div key={p.id} className="flex items-center gap-3 p-2.5 rounded-lg" style={{ backgroundColor: 'var(--bg-row)' }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold truncate text-gray-900">{p.prenom} {p.nom}</p>
              <p className="text-xs truncate text-gray-500">{p.detail || formatPhone(p.telephone)}</p>
            </div>
