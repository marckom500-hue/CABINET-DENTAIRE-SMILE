...
export default function Facturation() {
  ...
  const filtered = factures.filter(
    f => filtre === 'tous' || normalizeFactureStatus(f.statut) === filtre
  )
  ...
  const handleChangeStatut = async (facture, nouveauStatut) => {
    setChangingStatus(facture.id)
    try {
      await modifierFacture(facture.id, {
        ...facture,
        statut:  nouveauStatut,
        montant: Number(facture.montant),
      })
      const label = { attente:'En attente', paye:'Payée', annule:'Annulée' }[nouveauStatut] ?? nouveauStatut
      notify({ type: 'success', message: `Statut mis à jour : ${label}` })
    } catch {
      notify({ type: 'error', message: 'Erreur lors du changement de statut' })
    } finally {
      setChangingStatus(null)
  ...
  const handleExport = () => {
    setExporting(true)
    try {
      // Exporter les factures filtrées (respecte le filtre actif)
      exportComptableCSV(filtered, getPatientName)
      notify({ type: 'success', message: `Export réussi — ${filtered.length} facture(s) exportée(s)` })
    } catch (err) {
      console.error('Erreur export:', err)
      notify({ type: 'error', message: "Erreur lors de l'export" })
    } finally {
      setExporting(false)
    }
  }
  ...
  const getSelectClass = (statut) => {
    const n = normalizeFactureStatus(statut)
    if (n === FACTURE_STATUS.PAYE)   return 'bg-teal-100  text-teal-700  border-teal-200'
    if (n === FACTURE_STATUS.ANNULE) return 'bg-red-100   text-red-700   border-red-200'
    return 'bg-amber-100 text-amber-700 border-amber-200'
  }
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Bouton Export — visible si le rôle a accès aux patients ET à la facturation */}
          {canExport && (
            <button
              onClick={handleExport}
              disabled={exporting || filtered.length === 0}
              className="flex items-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
              title={`Exporter ${filtered.length} facture(s) en CSV / Excel`}
            >
              {exporting ? (
          ...
          {/* Bouton Nouvelle facture — écriture requise */}
          <PermissionGate module="facturation" requireWrite>
            <button
              onClick={openCreate}
              className="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {/* Indication dynamique pour l'export */}
        {canExport && filtre !== 'tous' && (
          <span className="text-xs text-gray-400 ml-1">
            → l'export portera sur les factures "{
              { paye:'Payées', attente:'En attente', annule:'Annulées' }[filtre]
            }" uniquement
          </span>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                const p              = f.patients
                const currentStatut  = normalizeFactureStatus(f.statut)
                const patientDisplay = p
                  ? `${p.prenom ?? ''} ${p.nom ?? ''}`.trim()
                  : getPatientName(f.patient_id)

                return (
                  <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                    {/* Patient — toujours visible (le hook usePatients est accessible
                        au comptable grâce au patch roles.js : patients.comptable = 'read') */}
                    <td className="px-4 py-3 font-medium text-gray-900">
                      {patientDisplay}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        {/* PDF — visible par tous ceux qui accèdent à la facturation */}
                        <button
                          onClick={() => setPreviewF(f)}
                          className="p-1.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors"
                          title="Aperçu / Télécharger PDF"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {/* Supprimer — écriture requise */}
                        <PermissionGate module="facturation" requireWrite>
                          <button
                            onClick={() => setConfirmD(f)}
                            className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                            title="Supprimer"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Modal
        ...
        title={editF ? 'Modifier la facture' : 'Nouvelle facture'}
        confirmOnClose
      >
        <div className="space-y-3">
          <div>
          ...
          <div>
            ...
            <select
              value={form.acte}
              onChange={e => set('acte')(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white"
            >
              <option value="">-- Choisir un acte --</option>
              {ACTES_OPTIONS.map(a => (
          ...
          <div>
            ...
            <p className={`text-xs mt-1 ${
              dateTouched ? (dateIsValid ? 'text-teal-600' : 'text-red-600') : 'text-gray-400'
            }`}>
              {dateTouched
                ? dateIsValid
                  ? 'Date valide'
                  : "Date invalide : elle ne peut pas être supérieure à aujourd'hui"
                : "Les dates futures sont bloquées dans le calendrier"}
            </p>
          </div>
      ...
      <ConfirmDialog
        isOpen={!!confirmD}
        onConfirm={async () => {
          try { await supprimerFacture(confirmD.id); setConfirmD(null) } catch {}
        }}
        onCancel={() => setConfirmD(null)}
        title="Supprimer la facture"
        message="Supprimer définitivement cette facture ?"
