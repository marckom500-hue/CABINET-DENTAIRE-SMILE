...
//                   const estConfirme = statut === RDV_STATUS.CONFIRME
...
import { useMissedAppointmentsDetector } from '../hooks/useMissedAppointmentsDetector'
import { RDV_STATUS, RDV_STATUS_META, normalizeRdvStatus } from '../lib/statuses'
import { RDV_STATES, RDV_ACTIONS, getPossibleActions, executeTransition } from '../lib/rdvStateMachine'
...
export default function MedecinRdv() {
  ...
  const handlePrevious = () => {
    const date = new Date(selectedDate)
    if (viewMode === 'jour') date.setDate(date.getDate() - 1)
    else if (viewMode === 'semaine') date.setDate(date.getDate() - 7)
    setSelectedDate(date.toISOString().split('T')[0])
  }
  ...
  // Exécuter une transition d'état
  const handleTransition = async (rdv, action) => {
    try {
      const currentState = normalizeRdvStatus(rdv.statut)
      const result = executeTransition(currentState, action, profile?.role || 'assistant', rdv)
      
      if (!result.success) {
        notify({ type: 'error', message: `Action non autorisée: ${result.error}` })
        return
      }
      
      const updateData = { statut: result.newState, ...result.updateData }
      const { error } = await supabase
        .from('rendez_vous')
        .update(updateData)
        .eq('id', rdv.id)
      ...
      const labelMap = {
        [RDV_ACTIONS.CONFIRMER]: 'confirmé ✓',
        [RDV_ACTIONS.TERMINER_PRESENT]: 'terminé ✓',
        [RDV_ACTIONS.MARQUER_ABSENT]: 'absent ✗',
        [RDV_ACTIONS.MARQUER_ABSENT_DEPART]: 'absent dès départ ✗',
        [RDV_ACTIONS.ANNULER]: 'annulé',
        [RDV_ACTIONS.RECTIFIER]: 'rectifié',
      }
      
      notify({ type: 'success', message: `RDV ${labelMap[action] || action}` })
      setTerminantRdv(null)
    } catch (err) {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {viewMode !== 'mois' && (
          <div className="flex items-center gap-2">
            <button onClick={handlePrevious} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <button onClick={handleToday} className="px-3 py-1.5 text-xs font-medium text-teal-600 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors">
              Aujourd'hui
            </button>
            <button onClick={handleNext} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </button>
      ...
      {/* Guide */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
        <strong>Machine à états:</strong> Programmé 
        <span className="font-semibold text-blue-600"> → Confirmer</span> 
        <span className="font-semibold text-blue-600"> → Terminer/Absent</span>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {rdvAffichees.map(r => {
                  ...
                  const s = RDV_STATUS_META[statut] ?? RDV_STATUS_META[RDV_STATUS.PROGRAMME]
                  const patient = r.patients
                  const possibleActions = getPossibleActions(statut, profile?.role || 'assistant', r)
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1.5 flex-wrap">
                          {possibleActions.map(action => {
                            ...
                            const isTerminate = [RDV_ACTIONS.TERMINER_PRESENT, RDV_ACTIONS.MARQUER_ABSENT].includes(action.action)
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (isTerminate) {
                                    setTerminantRdv({ ...r, nextAction: action.action })
                                  } else {
                                    handleTransition(r, action.action)
                                  }
                                }}
                                className={`px-2.5 py-1 text-xs font-medium shadow-md rounded-lg transition-all ${colors[action.color] || colors.gray}`}
                                title={action.label}
                              >
                                {action.label.substring(0, 15)}
                              </button>
                            )
                          })}
                        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-600">Confirmés</p>
          <p className="text-2xl font-bold text-green-600">
            {rdvAffichees.filter(r => normalizeRdvStatus(r.statut) === RDV_STATUS.CONFIRME).length}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-600">Terminés</p>
      ...
      {terminantRdv && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-4 z-50 pointer-events-none">
          <div className="absolute inset-0 bg-black/40 pointer-events-auto" onClick={() => setTerminantRdv(null)} />
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 pointer-events-auto space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">Terminer la consultation</h3>
            <p className="text-sm text-gray-600">
              Patient <strong>{terminantRdv?.patients?.prenom} {terminantRdv?.patients?.nom}</strong>
            </p>
            <div className="flex gap-3 pt-4">
              <button onClick={() => setTerminantRdv(null)} className="flex-1 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                Annuler
              </button>
              <button onClick={() => handleTransition(terminantRdv, RDV_ACTIONS.MARQUER_ABSENT)} className="flex-1 px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors">
                ✗ Absent
              </button>
              <button onClick={() => handleTransition(terminantRdv, RDV_ACTIONS.TERMINER_PRESENT)} className="flex-1 px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-lg transition-colors">
                ✓ Présent
              </button>
            </div>
          </div>
